...
}" @open-create-modal.window="show = true" @keydown.escape.window="show = false" x-show="show"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg dark:shadow-gray-900 w-[40rem] max-h-[90vh] overflow-y-auto"
        ...
        <form
                ...
                if (!res.ok) {
                    return res.text().then(text => {
                        ...
                        window.dispatchEvent(new CustomEvent('show-toast', {
                            detail: { type: 'error', message: 'Account Creation failed: ' + res.status }
                        }));
            ...
            .then(data => {
                    ...
                    // notify dashboard/sidebar to refresh stats immediately in this session
                    if (window.Livewire && typeof window.Livewire.emit === 'function') {
                        window.Livewire.emit('statsUpdated');
                    }
                    window.dispatchEvent(new CustomEvent('show-toast', {
                        detail: { type: 'success', message: data.message || 'Account Created successfully!' }
                    }));
                } else {
                    window.dispatchEvent(new CustomEvent('show-toast', {
                        detail: { type: 'error', message: data.message || 'Account Creation failed' }
                    }));
                }
                resetForm();
            })
        ...
        "
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit"
                    class="w-40 bg-[#009639] dark:bg-[#00b347] text-white px-4 py-2 rounded-md hover:bg-[#007a2e] dark:hover:bg-[#009639] focus:outline-none focus:ring-2 focus:ring-green-400 dark:focus:ring-green-500">
                    Create Account
                </button>
            </div>
        </form>
