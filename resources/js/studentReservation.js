...
window.reservationApp = function (rooms) {
    return {
        ...
        transformReservations(data) {
            ...
            data.reservations.forEach(res => {
                ...
                all.push({
                    ...
                    time_display: timeLabels[startKey] || `${startKey}-${res.end_time.substring(0, 5)}`
                });
            });
            ...
            data.blocked_slots.forEach(blocked => {
                ...
                all.push({
                    ...
                    time_display: timeLabels[startKey] || `${startKey}-${blocked.end_time.substring(0, 5)}`
                });
            });
        ...
        getAvailableSlotsForDay(date) {
            const dayReservations = this.reservations.filter(res => res.date === date);
            const allSlots = [
            ...
            return allSlots.map(slot => {
                const reserved = dayReservations.find(res =>
                    res.start_time.substring(0, 5) === slot.start
                );
                return {
                    ...slot,
        ...
        selectTimeSlot(date, slot) {
            if (!slot.isAvailable || this.calendarDays.find(d => d.date === date)?.isPast) {
                return;
            }
